...
def clean_text(text):
    ...
    text = re.sub(r'\[.*?\]', '', text)  # Remove all footnotes inside square brackets
    text = text.replace("\u00a0", " ").replace("\u2013", "-")  # Normalize spaces and dashes
...
def format_term(term):
    ...
    term = term.replace("\u2013", "-")
    return " - ".join(part.strip() for part in term.split("-")) if "-" in term else term.strip()
...
def scrape_presidents():
    ...
    for president in table.find_all("tr")[1:]:
        ...
        try:
            ...
            vice_president = clean_vice_president(clean_text(vice_president_tag.get_text(separator=" ")))
